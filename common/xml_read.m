...
if (Debug)   % in debuging mode allow crashes
  ...
  try
    ...
    error('Unable to parse XML file %s: %s.',xmlfile, getReport(ME));
  end
end
...
[sname LeafNode] = NodeName(node, Pref.KeepNS);
s = [];
...
if (LeafNode)
  ...
  if (LeafNode>0) % supported leaf node types
    s = char(node.getData);
    if (isspace(s(1)) || isspace(s(end))), s = strtrim(s); end
    if (LeafNode==1), s=str2var(s, Pref.Str2Num); end
  end
...
if (node.hasAttributes && Pref.ReadAttr)
  ...
  for iAttr = 1:Attr.getLength    % for each attribute
    ...
    value = str2var(value, Pref.Str2Num);      % convert to number if possible
...
%  === str2var Function ==================================================
%  =======================================================================
function val=str2var(str, option)
