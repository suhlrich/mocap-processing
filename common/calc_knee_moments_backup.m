function [KAM KFM KAM_ma KFM_ma time_mat] = calc_knee_moments(T_TCS_ACSstruct,markers,forces,leg,BW_kg,h_m,step_frames,time)
...
KAM = zeros(size(markers,1),1) ; KFM = KAM ; KAM_ma = KAM ; KFM_ma = KAM ; 
...
for i = 1:size(markers,1)
    ...
    if max(abs(T_TCS_ACS(1:3,4)))>10 % this is a bad way to see if T_TCS_ACS was recorded in mms
    ...
    r = COP - kjc ;
    KM = cross(F,r);
    KM_ACS = T_lab_ACS(1:3,1:3)'*KM ;
    F_ACS = T_lab_ACS(1:3,1:3)'*F ;
...
if nargin > 6
    ...
    KFM_ma_mat = KAM_mat ; Fz_mat = KAM_mat ; KM_mat = KAM_mat ;
