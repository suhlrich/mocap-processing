function [T_TCS_ACS, T_TCS_ACS_ank] = calc_T_static_KneeAnkle(header_static,data_static)
...
% Average of the static marker data    
...
% Create transormation matrix from tracking coord to anatomical coord
        r_kjc = (r_mkne+r_knee) /2 ; % Right knee joint center
        r_ajc = (r_mank+r_ankl) /2 ; % Right ankle joint center
        l_kjc = (l_knee+l_mkne) /2 ; % Left knee joint center
        l_ajc = (l_ankl+l_mank) /2 ; % Left knee ankle center
        ...
        r_yacs_ank = r_mank-r_ajc ; r_yacs_ank = r_yacs_ank/norm(r_yacs_ank) ;
        r_xacs_ank = cross(r_kjc-r_ajc,r_yacs_ank) ; r_xacs_ank = r_xacs_ank/norm(r_xacs_ank) ;
        r_zacs_ank = cross(r_xacs_ank,r_yacs_ank) ; r_zacs_ank = r_zacs_ank/norm(r_zacs_ank) ;
        l_yacs_ank = l_mank-l_ajc ; l_yacs_ank = l_yacs_ank/norm(l_yacs_ank) ;
        l_xacs_ank = cross(l_kjc-l_ajc,l_yacs_ank) ; l_xacs_ank = l_xacs_ank/norm(l_xacs_ank) ;
        l_zacs_ank = cross(l_xacs_ank,l_yacs_ank) ; l_zacs_ank = l_zacs_ank/norm(l_zacs_ank) ;
...
%         l_zacs = cross(l_xacs,l_yacs) ; l_zacs = l_zacs/norm(l_zacs) ;
        ...
        r_T_lab_ACS_ank = [[r_xacs_ank';0],[r_yacs_ank';0],[r_zacs_ank';0],[r_ajc';1]] ;
        l_T_lab_ACS_ank = [[l_xacs_ank';0],[l_yacs_ank';0],[l_zacs_ank';0],[l_ajc';1]] ;
...
% Tracking coordinate system        
        ...
        l_T_lab_TCS_ank = [[l_xtcs';0],[l_ytcs';0],[l_ztcs';0],[l_ankl';1]] ; 
        r_T_lab_TCS_ank = [[r_xtcs';0],[r_ytcs';0],[r_ztcs';0],[r_ankl';1]] ; 

        T_TCS_ACS.r = inv(r_T_lab_TCS)*r_T_lab_ACS ;
        T_TCS_ACS.l = inv(l_T_lab_TCS)*l_T_lab_ACS ;
        
        T_TCS_ACS_ank.r = inv(r_T_lab_TCS_ank)*r_T_lab_ACS_ank ;
        T_TCS_ACS_ank.l = inv(l_T_lab_TCS_ank)*l_T_lab_ACS_ank ;
