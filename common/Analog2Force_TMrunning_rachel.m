function [forces_proc_meters] = Analog2Force_TMrunning_rachel(forcesV,TreadmillCalibMatrix,filtfreq,samp_rate,threshold_high,threshold_low) ;
...
for i=1:2
    ...
    if i==1 % Convert the left FP moments (force plate #2) into the right FP (force plate #1) reference frame, cross(r,F).
        r=[0 -0.7299-0.2413 0];
...
% COPz=zeros(length(ForMom),1); % COPz == 0 because lab coordinate system is at treadmill surface
% SCOTT CHANGED THIS 1/2/19 when we figured out that the location of TM
% origin is below wearplates. Uncomment top line to go back to how we were
% doing it before.
