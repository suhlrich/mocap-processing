function [forces_proc_meters] = Analog2Force_TMrunning(forcesV,TreadmillCalibMatrix,filtfreq,samp_rate,threshold_high,threshold_low,freq_notch,plotOn) ;
...
if isempty(filtfreq) == 0
    if exist('freq_notch')
        [B_n,A_n] = notchFilter(freq_notch,samp_rate,0.007) ; % Notch filter to knock out belt oscillation (really just for M/L). 25Hz is usually good
        forcesV = filtfilt(B_n,A_n,forcesV) ;
    end
...
COPz=h*ones(length(ForMom),1); % COPz == h because lab coordinate system is at treadmill surface (below the wearplate of thickness h). verified 2/25/22
