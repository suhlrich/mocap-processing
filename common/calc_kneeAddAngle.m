function [kneeAngle_interp_traces, kneeAngle_interp_u, kneeAngle_peak, kneeAngle_peak_u] = ... 
    calc_kneeAddAngle(markers, leg, recframes,goodtrials)
...
if exist('recframes')
    ...
    for i = 1:size(recframes,1)
        if ismember(i,goodtrials)
            ...
            kneeAngle_interp_traces(j,:) = interp101(calcInternalKAA(recframes(i,1:nFrames),HJC,KJC,AJC,latFemCond,FemCondVec,k_fem,k_tib,leg)) ;
            kneeAngle_peak(j) = max(kneeAngle_interp_traces(j,:)) ;
            j = j+1 ;
        end
    end
    ...
    kneeAngle_interp_traces = calcInternalKAA(1:length(KJC),HJC,KJC,AJC,latFemCond,FemCondVec,k_fem,k_tib,leg) ;
...
function KneeAdd = calcInternalKAA(rg,HJC,KJC,AJC,latFemCond,FemCondVec,k_fem,k_tib,leg)
    for i = 1:length(rg)
        j = rg(i) ;
        ...
        % Floating axis
        cosb(i,:) = dot(e1(i,:),e3(i,:));
        if leg == 'r' % For right leg
            KneeAdd(i) = rad2deg(acos(cosb(i))+pi/2) - 180;
        else % For left leg
            KneeAdd(i) = rad2deg(acos(cosb(i))-pi/2);
        end
    end
end